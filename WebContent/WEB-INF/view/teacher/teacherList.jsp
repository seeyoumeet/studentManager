...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        columns: [[  
				{field:'chk',checkbox: true,width:50},
 		        ...
 		        {field:
					...
					title:'课程',
					width:500, 
 		        	formatter: function(value,row,index){		// 对courseList进行格式化改造
 						if (row.courseList){
	        ...
	        toolbar: "#toolbar"		// 设置datagrid的工具栏
	    });
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectLength == 0){
				...
				// 取出所有选中行的number列表(数据库中的number)
            	var numbers = [];
            	$(selectRows).each(function(i, row){
            		numbers[i] = row.number;
            	});
				
            	$.messager.confirm("消息提醒", "将删除与教师相关的所有数据，确认继续？", function(r){	// 在弹出框信息选择中向服务器发送Ajax请求,判断删除结果
            		if(r){
            			$.ajax({
							type: "post",
